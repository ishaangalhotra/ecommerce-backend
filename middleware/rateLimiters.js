 ...
 * 3. Machine learning-based anomaly detection (placeholder hooks)
...
const createStore = (windowMs = RATE_LIMIT_CONFIG.DEFAULT_WINDOW) => {
  try {
    // Preferred: use simple redisClient if flagged
  ...
  // Fallback to advanced redis client wrapper
  try {
    if (redis?.client) {
      return new RedisStore({
        // rate-limit-redis accepts either `client` (ioredis/redis) or sendCommand
        // Provide a robust sendCommand with light circuit-breaker style checks
          ...
          try {
            ...
            if (duration > 100) {
              logger.warn('Slow Redis command', {
                command: Array.isArray(args) ? args[0] : String(args),
                duration: `${duration.toFixed(2)}ms`
              });
            }
            if (redis.trackSuccess) redis.trackSuccess();
            return result;
          } catch (err) {
            if (redis.trackFailure) redis.trackFailure(err);
  ...
  return undefined; // rate-limit will fallback to memory store
};
...
function validateConfig(config) {
  ...
  if (dynamicLimiting != null && typeof dynamicLimiting !== 'boolean') {
  ...
  if (requestWeighting != null && typeof requestWeighting !== 'boolean') {
  ...
  if (tokenBucket != null && typeof tokenBucket !== 'boolean') {
  ...
  if (tokenBucket && (dynamicLimiting || requestWeighting)) {
    throw new RateLimitConfigError(
      'Token bucket cannot be combined with dynamic limiting or request weighting',
      { tokenBucket, dynamicLimiting, requestWeighting }
    );
  }
}
...
function isWhitelisted(req) {
  ...
  if (req.headers['x-internal-request'] === process.env.INTERNAL_REQUEST_SECRET) return true;
  return false;
}
...
async function calculateDynamicLimit(baseLimit) {
  try {
    const load = await getSystemLoad(); // 0..1
    const loadFactor = Math.min(1, Math.max(0, load / RATE_LIMIT_CONFIG.LOAD_FACTOR_THRESHOLD));
    const adjustmentFactor = 1 - loadFactor * loadFactor; // quadratic easing
    return Math.max(
...
async function calculatePenaltyMultiplier(clientId) {
  try {
    ...
    const level = Math.min(parseInt(penaltyLevel, 10) || 0, RATE_LIMIT_CONFIG.MAX_PENALTY_LEVEL);
...
async function updatePenaltyLevel(clientId) {
  try {
    ...
    const current = parseInt(await client.get(key), 10) || 0;
    const newLevel = Math.min(current + 1, RATE_LIMIT_CONFIG.MAX_PENALTY_LEVEL);
    // 1 hour TTL
    if (client.setEx) {
      await client.setEx(key, 3600, String(newLevel));
    } else if (client.setex) {
      await client.setex(key, 3600, String(newLevel));
    } else {
      await client.set(key, String(newLevel));
      if (client.expire) await client.expire(key, 3600);
    }
  } catch (error) {
...
function calculateRequestWeight(req) {
  if (REQUEST_CLASSIFIERS.HEAVY_REQUESTS.some((p) => req.path.startsWith(p))) {
    return RATE_LIMIT_CONFIG.REQUEST_WEIGHTS.HEAVY;
  }
  if (REQUEST_CLASSIFIERS.LIGHT_REQUESTS.some((p) => req.path.startsWith(p))) {
...
async function trackSuspiciousActivity(req) {
  try {
    ...
    if (rateLimitInfo.current >= suspiciousThreshold) {
      const key = `${RATE_LIMIT_CONFIG.REDIS_PREFIX}suspicious:${anonymizeClientId(
        req.rateLimitContext?.clientId || req.ip
      )}`;
      if (client.incr) await client.incr(key);
      if (client.expire) await client.expire(key, 3600); // 1h
...
function anonymizeEmail(email) {
  ...
  return `${name}${'*'.repeat(Math.max(0, name.length - 1))}@${domain}`;
}

/**
 * Token bucket store (lightweight, using Redis hash)
 */
function createTokenBucketStore(max, windowMs) {
  const capacity = Math.floor(max * 1.5); // burst
  ...
  return {
    async increment(key) {
      try {
        ...
        const hgetall = client.hGetAll ? await client.hGetAll(bucketKey) : (await client.hgetall(bucketKey)) || {};
        const lastRefill = parseInt(hgetall.lastRefill, 10) || now;
        const tokens = parseFloat(hgetall.tokens) || capacity;
        ...
        if (newTokens >= 1) {
          const newState = { lastRefill: String(now), tokens: String(newTokens - 1) };
          if (client.hSet) {
            await client.hSet(bucketKey, newState);
            if (client.pExpire) await client.pExpire(bucketKey, windowMs);
            else if (client.expire) await client.expire(bucketKey, Math.ceil(windowMs / 1000));
          } else {
            // legacy
            await client.hmset(bucketKey, newState);
            if (client.pexpire) await client.pexpire(bucketKey, windowMs);
            else if (client.expire) await client.expire(bucketKey, Math.ceil(windowMs / 1000));
          }
        ...
        // Not enough tokens: return next refill ETA
...
const createLimiter = (options = {}) => {
  ...
  const enhancedKeyGenerator =
    ...
    ((req) => {
      ...
      if (req.user?.id) {
        ...
        if (req.user.role) components.push(`role:${req.user.role}`);
      }
      if (req.route?.path) {
  ...
  const enhancedHandler = async (req, res, next) => {
    ...
    try {
      ...
      if ((rateLimitInfo?.current || 0) > adjustedLimit * 2) {
        const severity = (rateLimitInfo?.current || 0) > adjustedLimit * 5 ? 'critical' : 'high';
  ...
  if (customStore) {
    ...
    // We wrap token bucket using a custom store via keyGenerator externally in route usage;
    // For express-rate-limit, we still need a store; we can rely on memory but use our own check before.
    // To integrate properly with express-rate-limit, we keep normal store and use token bucket outside typically.
    // As a pragmatic approach, we still provide a Redis store for counters and rely on our token bucket for weight logic elsewhere.
  ...
  return async (req, res, next) => {
    ...
    try {
      req.rateLimitContext = {
        requestId,
        startTime: Number(startTime) / 1e6,
      ...
      limiter(req, res, (err) => {
        ...
        if (req[requestPropertyName]) {
          res.set({
            ...
            'X-RateLimit-Request-Weight': String(req.rateLimitContext.requestWeight)
          });
        }
...
const rateLimiters = {
  ...
  // Backward compatible login limiter
  ...
  search: createLimiter({
    ...
    requestWeighting: true,
    skip: (req) => req.user?.plan === 'premium' || req.user?.plan === 'enterprise'
  }),
...
const smartRateLimit = (options = {}) => {
  ...
  return async (req, res, next) => {
    try {
      // Bypass header for internal services
      ...
      if (
        enableAuth &&
        (path.includes('/auth/') || path.includes('/login') || path.includes('/signin'))
      ) {
      ...
      if (
        method === 'post' &&
        (path.includes('/upload') || (req.headers['content-type'] || '').includes('multipart/form-data'))
      ) {
...
const resetRateLimit = async (identifier, limiterType = 'global') => {
  try {
    ...
    const keysCmd = `${RATE_LIMIT_CONFIG.REDIS_PREFIX}*${identifier}*`;
    const keys = client.keys ? await client.keys(keysCmd) : [];
    if (keys.length > 0) {
...
const getRateLimitStats = async (identifier) => {
  try {
    ...
    for (const key of keys) {
      const ttl = client.ttl ? await client.ttl(key) : null;
      const value = client.get ? await client.get(key) : null;
      if (key.includes('penalty:')) {
        stats.penalties[key] = { value: parseInt(value, 10), ttl };
      } else if (key.includes('suspicious:')) {
        stats.suspicious[key] = { value: parseInt(value, 10), ttl };
      } else {
        stats.limits[key] = { value: parseInt(value, 10), ttl };
      }
    }
    return stats;
  } catch (error) {
