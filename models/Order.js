...
orderSchema.virtual('isReturned').get(function() {
  return this.status === 'returned' || (this.returnInfo && this.returnInfo.isReturned);
});
...
orderSchema.virtual('canBeReturned').get(function() {
  const deliveredDate = this.deliveryTracking && this.deliveryTracking.deliveredAt;
...
orderSchema.pre('save', async function(next) {
  try {
    ...
    if (this.isNew && (!this.customerInfo || !this.customerInfo.name)) {
...
orderSchema.methods.calculateDeliveryTime = function() {
  if (!this.deliveryTracking || !this.deliveryTracking.deliveredAt || !this.createdAt) return null;
...
orderSchema.methods.initiateRefund = async function(amount, reason) {
  this.paymentResult = this.paymentResult || {};
